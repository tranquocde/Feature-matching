...
def get_gradient_histogram_vec_from_patch(
    ...
    bins = np.arange(-8, 9, 2) * np.pi / 8 # 9 numbers => 8 spaces: 
    wgh = np.zeros(128)
    counter = 0
    for i in range(h // 4):
        for j in range(w // 4):
            ...
            counter += 1
    # In NumPy, np.newaxis is a special constant 
    # that can be used to increase the dimensionality 
    # of an array by adding an axis of length 1. It is
    #  often used in array indexing or broadcasting
    #  operations to reshape or align arrays.
    wgh = wgh[:, np.newaxis]
...
def get_feat_vec(
    ...
    #padding the magnitudes matrix with pad_size = feature_width/2 
    #padding the orientations matrix with pad_size = feature_width/2
    #handle the case the interest point is near the border
    ...
    # new_c and new_r after padding
    new_c, new_r = c + feature_width // 2, r + feature_width // 2
    
    #starting column index and starting row index (substract the pad_size from new_c , new_r)
    #minus 1 because 0-based index
    start_i, start_j = new_r - (feature_width // 2 - 1), new_c - (feature_width // 2 - 1)

    # get the patch for the specific interest point in
    #  padded magnitudes and padded orientations
...
def get_SIFT_descriptors(
    ...
    k = len(X) # number of interest points
    fvs = np.zeros((k, 128)) # feature vectors for interest points
    Ix, Iy = compute_image_gradients(image_bw=image_bw) # Gradients in x-axis and y-axis
    #compute magnitudes and orientations matrix given the gradients matrices
    magnitudes, orientations = get_magnitudes_and_orientations(Ix, Iy)
    for i in range(k):
        #get features vector for i-th interest point
        fvs[i] = get_feat_vec(X[i], Y[i], magnitudes=magnitudes,\
            orientations=orientations, feature_width=feature_width).ravel()
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return fvs
