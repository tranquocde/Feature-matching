...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Ix = - signal.convolve2d(image_bw,SOBEL_X_KERNEL,mode='same')
    Iy = - signal.convolve2d(image_bw,SOBEL_Y_KERNEL,mode='same')
...
def compute_harris_response_map(
    ...
    # print(S_xx.shape,S_yy.shape,S_xy.shape)
...
def nms_maxpool_pytorch(
    ...
    # torch.where() function selects elements from the
    # input_tensor when the corresponding condition is True, and
    # elements from the other_tensor when the condition is False
    # applying squeeze() to arr, we remove the dimensions with a length of 1
    bin_R = torch.where(maxpooled_R == R, R, 0).squeeze().numpy()

    # gets the indices of the R-values in ascending order
    # https://stackoverflow.com/questions/48135736/what-is-an-intuitive-explanation-of-np-unravel-index
    # unravel_index:  find the index of values in the matrix with shape.
    ind = np.unravel_index(np.argsort(bin_R, axis=None), bin_R.shape)
    y, x = np.flip(ind[0])[:k], np.flip(ind[1])[:k]
    confidences = bin_R[y, x]
