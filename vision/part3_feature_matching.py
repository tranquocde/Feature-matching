...
def compute_feature_distances(
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    features1_euclidean = np.sum(features1 ** 2, axis=1, keepdims=True) # (n1,1)
    features2t = features2.T
    features2_euclidean = np.sum(features2t ** 2, axis=0, keepdims=True) #(1,n2)
...
def match_features_ratio_test(
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    threshold = 0.76 # ???? how to find the threshold.
    dists = compute_feature_distances(features1, features2)
    sorted_dists = np.sort(dists, axis=1) #2d array sorted distance by row
    # print(sorted_dists)
    sorted_dist_ind = np.argsort(dists, axis=1) #2d array (index of) sorted distance by row
    #compute the NNDR (nearest neighbor distance ratio)
    #ratio of nearest and second neighbor distances.
    NNDR = sorted_dists[:, 0] / sorted_dists[:, 1] #1d array
    # print(NNDR)
    allowed_matches_count = (NNDR <= threshold).sum() #scalar
    # print(allowed_matches_count)
    confidence_ind = np.argsort(NNDR)[:allowed_matches_count]
    if len(confidence_ind) > 0:
        confidences = NNDR[confidence_ind]
        matches = np.zeros((len(confidences), 2), dtype=np.int32)
